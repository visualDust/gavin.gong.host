...
在阅读本文前，请先阅读{% post_link BiSeNet-Bilateral-Segmentation-Network-for-Real-time-Semantic-Segmentation BiSeNet %}。

该论文提出{% post_link BiSeNet-Bilateral-Segmentation-Network-for-Real-time-Semantic-Segmentation BiSeNet %} 被证明是不错的双路实时分割网络。不过，在BiSeNet中：
...
注：上图中红色虚线框中的部分是新提出的STDC network；ARM表示注意力优化模块（Attention Refinement Module），FFM表示特征融合模块（Feature Fusion Module）。这两个模块是在{% post_link BiSeNet-Bilateral-Segmentation-Network-for-Real-time-Semantic-Segmentation BiSeNet %} 就已经存在的设计。
