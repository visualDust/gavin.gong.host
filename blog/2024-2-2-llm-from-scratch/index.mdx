...
A vocabulary table is a list or dictionary where each unique word or  token in the training dataset is assigned a unique integer index. This  table forms the basis for converting text data into numerical form that a machine learning model can understand. In the context of natural language processing (NLP)  and machine learning models, especially those dealing with text data, vocabulary table is a critical component. It is essentially a mapping of words (or tokens)  to numerical values, enabling the model to process and understand text  data. As you can see, code in `convert_data.py`  generates vocabulary table file by counting every character that appears in training data:
...
`convert_data.py` actually ignores important steps when generating vocabulary  table, that is, **Handling Rare Words**. Rare words may appear only once in whole training set, a simple approach is to split rare words into subword units if a subword tokenization strategy is used, or alternatively, rare words can be mapped to a special "unknown" token.
...
1. **Error Propagation** or, **Exposure Bias**: Since the above architecture predicts new token based on previously generated tokens, and the newly generated token later becomes input for the next comming tokens, errors made in early predictions can propagate and magnify in later stages. During training, the model always sees perfect previous steps (due to teacher forcing), but during inference, it must rely on its own predictions, which might not be perfect. This discrepancy can lead to compounding errors in the generated sequence.
