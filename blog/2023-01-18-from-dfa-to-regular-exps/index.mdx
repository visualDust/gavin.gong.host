...
==Example== 请设计DFA，在任何由0和1构成的串中，接受含有01子串的全部串。
...
==Example== 若 $\Sigma = \{0,1\}$，给出接受全部含有奇数个1的串DFA。
...
==Example== 若 $\Sigma = {0,1}$，给出DFA接受全部含有偶数个0和偶数个1的串。
...
==Example==：设计DFA接受 $L = \{w\in\{0,1\}^* | w 倒数3个字符至少有一个是1 \}$。
...
==Example==：设计 $L = \{w\in\{0,1\}^* | w 倒数3个字符至少有一个是1 \}$ 对应的NFA。
